...
def processMatchJson(match_data, gameid):
    ...
    for partic in match_data['info']['participants']:
        ...
        if unitlist:
            ...
            num_units = 0
        partic_data = [game_dtime, gameid, puuid, place, traitlist, unitlist, dmg, num_units]
        flat_data_list.append(partic_data)
    flatdf = pd.DataFrame(flat_data_list, columns = ['Game Date','GameID','PUUID', 'Place', 'Traits', 'Units', 'Damage', 'TeamSize'])
    return flatdf
